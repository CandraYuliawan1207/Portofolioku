<?php include 'includes/header.php'; ?>
<main style="margin-bottom: 0;">
    <section class="hero text-black text-center d-flex align-items-center justify-content-center" style="height: 80vh; position: relative;">
        <!-- Animasi Wave -->
        <img src="https://threedio-prod-var-cdn.icons8.com/vb/preview_sets/previews/JpZQHPMv0qeSmJHp.webp"
            alt="Waving silhouette man"
            style="position: absolute; top: 12%; left: 50%; transform: translateX(-50%); width: 150px; height: auto;"
            data-aos="fade-down" data-aos-duration="1000" />
        <div data-aos="fade-up" data-aos-delay="500">
            <h1 class="display-4 fw-bold">Candra Yuliawan</h1>
            <p class="lead">Office Administration | Web Developer Enthusiast</p>
            <a href="projects.php" class="btn btn-warning mt-3">Lihat Proyek Saya</a>
        </div>
    </section>

</main>

<?php include 'includes/footer.php'; ?>